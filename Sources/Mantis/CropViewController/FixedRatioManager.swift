...
extension FixedRatioManager {
    ...
    private func contains(ratioItem: RatioItemType) -> Bool {
        var contains = false
        for ratio in ratios {
            if ratio.nameH == ratioItem.nameH || ratio.nameV == ratioItem.nameV {
                contains = true
            }
        }
        return contains
    ...
    private func appendToTail(ratioItem: RatioItemType?) {
        guard let ratioItem, contains(ratioItem: ratioItem) == false else { return }
        ratios.append(ratioItem)
    }

    private func appendToTail(ratioItems: [RatioItemType]) {
        for item in ratioItems {
            appendToTail(ratioItem: item)
        }
    }

    private func appendToTail(items: [(width: Int, height: Int)]) {
        for item in items {
            let ratioItem = RatioItemType(
                nameH: String("\(item.width):\(item.height)"),
                ratioH: Double(item.width) / Double(item.height),
                nameV: String("\(item.height):\(item.width)"),
                ratioV: Double(item.height) / Double(item.width)
            )
